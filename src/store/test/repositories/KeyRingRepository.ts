...
export class ExpoKeyRingRepository implements KeyRingRepository {
    ...
    async getPersistedKeyPair(publicKey: string): Promise<Keypair | null> {
        ...
        try {
            ...
            throw new Error(
                `Failed to parse secret key for public key ${publicKey}: ${error instanceof Error ? error.message : 'unknown error'
                }`,
            );
        }
    }
    ...
    async getAllPersistedKeyPairs(): Promise<Keypair[]> {
        ...
        return rows.map((row) => {
            try {
                ...
                throw new Error(
                    `Failed to parse secret key for public key ${row.publicKey}: ${error instanceof Error ? error.message : 'unknown error'
                    }`,
                );
            }
        });
    ...
    async getLatestKeyPair(): Promise<Keypair | null> {
        ...
        try {
            ...
            throw new Error(
                `Failed to parse latest secret key for public key ${row.publicKey}: ${error instanceof Error ? error.message : 'unknown error'
                }`,
            );
        }
    }
